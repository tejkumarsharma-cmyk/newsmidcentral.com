...
const variantClasses = {
  ...
  'editorial-bar': {
    shell: 'border-b border-[#640D5F]/20 bg-white/95 text-[#640D5F] backdrop-blur-xl',
    logo: 'rounded-full border border-[#640D5F]/30 bg-white shadow-sm',
    active: 'bg-[#640D5F] text-white',
    idle: 'text-[#640D5F]/80 hover:bg-[#f8f4ff] hover:text-[#640D5F]',
    cta: 'rounded-full bg-[#640D5F] text-white hover:bg-[#4a0a47]',
    mobile: 'border-t border-[#640D5F]/20 bg-white',
  },
  'floating-bar': {
          ...
          {isEditorial ? (
            <div className="hidden min-w-0 flex-1 items-center gap-6 xl:flex">
              {primaryNavigation.map((task) => {
                const isActive = pathname.startsWith(task.route)
                return (
                  <Link key={task.key} href={task.route} className={cn('text-sm font-semibold uppercase tracking-[0.18em] transition-colors', isActive ? style.active : style.idle)}>
                    {task.label}
                  </Link>
                )
              })}
